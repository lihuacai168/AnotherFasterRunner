...
import { addVisit, queryVisit, removeVisit, updateVisit, queryVisitVerboseName, queryVisitListDisplay, queryVisitDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateVisit(value, current_id);
      hide();
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
                      ...
                      message: '请求方法为必填项',
                             ...
                             valueEnum:{"GET":"GET","POST":"POST","PUT":"PUT","PATCH":"PATCH","DELETE":"DELETE","OPTION":"OPTION"}
                        },
                      {
                             title: '请求体',
                              ...
                              render: (text, record) => (
                                <>
                                  ...
                                  <EditOutlined title="编辑" className="icon" onClick={async () => {
                                   record.create_time = record.create_time === null ? record.create_time : moment(record.create_time);
  ...
  useEffect(() => {
    queryVisitListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryVisit, table_columns, 'visit-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索visit" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'visit-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryVisit({ ...params, sorter, filter })}
