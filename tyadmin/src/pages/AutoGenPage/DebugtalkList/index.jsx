...
import { addDebugtalk, queryDebugtalk, removeDebugtalk, updateDebugtalk, queryDebugtalkVerboseName, queryDebugtalkListDisplay, queryDebugtalkDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateDebugtalk(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryDebugtalkDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryDebugtalkListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryDebugtalk, table_columns, '驱动库-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索驱动库" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '驱动库-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryDebugtalk({ ...params, sorter, filter })}
