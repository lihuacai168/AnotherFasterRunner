...
import { addCaseStep, queryCaseStep, removeCaseStep, updateCaseStep, queryCaseStepVerboseName, queryCaseStepListDisplay, queryCaseStepDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryCase, queryCaseVerboseName} from '@/pages/AutoGenPage/CaseList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCaseStep(value, current_id);
      hide();
            ...
            hideInForm: true,
                      ...
                      message: 'case为必填项',
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, caseForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, caseVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryCaseStepDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryCaseStepListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [caseForeignKeyList, setCaseForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryCase({all: 1}).then(value => {
                                     setCaseForeignKeyList(value);
                                });
                                }, []);
                                const [caseVerboseNameMap, setCaseVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryCaseVerboseName().then(value => {
                                    setCaseVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCaseStep, table_columns, '用例信息 Step-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索用例信息 Step" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用例信息 Step-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCaseStep({ ...params, sorter, filter })}
