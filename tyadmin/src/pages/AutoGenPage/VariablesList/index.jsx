...
import { addVariables, queryVariables, removeVariables, updateVariables, queryVariablesVerboseName, queryVariablesListDisplay, queryVariablesDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateVariables(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryVariablesDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryVariablesListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryVariables, table_columns, '全局变量-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索全局变量" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '全局变量-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryVariables({ ...params, sorter, filter })}
