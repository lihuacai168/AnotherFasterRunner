...
import { addProject, queryProject, removeProject, updateProject, queryProjectVerboseName, queryProjectListDisplay, queryProjectDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateProject(value, current_id);
      hide();
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
            ...
            hideInForm: true,
                      ...
                      {
                             title: 'yapi的openapi url',
                             
                             initialValue: "",
                             dataIndex: 'yapi_base_url',
                      ...
                      {
                             title: 'yapi openapi的token',
                             
                             initialValue: "",
                             dataIndex: 'yapi_openapi_token',
                      ...
                      {
                             title: 'jira_bearer_token',
                             
                             initialValue: "",
                             dataIndex: 'jira_bearer_token',
  ...
  useEffect(() => {
    queryProjectDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryProjectListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryProject, table_columns, '项目信息-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索项目信息" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '项目信息-select')
                    }
                  }}
                  selectedKeys={[]}
                >
