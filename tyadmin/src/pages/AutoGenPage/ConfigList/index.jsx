...
import { addConfig, queryConfig, removeConfig, updateConfig, queryConfigVerboseName, queryConfigListDisplay, queryConfigDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateConfig(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryConfigDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryConfigListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryConfig, table_columns, '环境信息-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索环境信息" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '环境信息-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryConfig({ ...params, sorter, filter })}
