...
import { addAPI, queryAPI, removeAPI, updateAPI, queryAPIVerboseName, queryAPIListDisplay, queryAPIDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateAPI(value, current_id);
      hide();
        ...
        hideInSearch: true,
            ...
            hideInForm: true,
                        ...
                        },
                      {
                             ...
                             valueEnum:{"0":"未知","1":"成功","2":"失败","3":"自动成功","4":"废弃"}
                        },
                      {
                             title: 'yapi的分组id',
                             
                             initialValue: 0,
                             dataIndex: 'yapi_catid',
                             valueType: 'digit',
                             rules: [
  ...
  useEffect(() => {
    queryAPIListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryAPI, table_columns, '接口信息-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索接口信息" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '接口信息-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryAPI({ ...params, sorter, filter })}
