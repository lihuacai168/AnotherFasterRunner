...
import { addUserToken, queryUserToken, removeUserToken, updateUserToken, queryUserTokenVerboseName, queryUserTokenListDisplay, queryUserTokenDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryUserInfo, queryUserInfoVerboseName} from '@/pages/AutoGenPage/UserInfoList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateUserToken(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryUserTokenDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryUserTokenListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryUserToken, table_columns, '用户登陆token-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索用户登陆token" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用户登陆token-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryUserToken({ ...params, sorter, filter })}
