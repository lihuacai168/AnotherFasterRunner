...
import { addCase, queryCase, removeCase, updateCase, queryCaseVerboseName, queryCaseListDisplay, queryCaseDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateCase(value, current_id);
      hide();
        ...
        hideInSearch: true,
            ...
            hideInForm: true,
                        ...
                        },
                      {
                             ...
                             valueEnum:{"1":"冒烟用例","2":"集成用例","3":"监控脚本","4":"核心用例"}
                        },
                          {
                              title: '操作',
                              dataIndex: 'option',
  ...
  useEffect(() => {
    queryCaseListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryCase, table_columns, '用例信息-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索用例信息" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用例信息-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryCase({ ...params, sorter, filter })}
