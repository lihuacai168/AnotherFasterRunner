...
from httprunner.utils import create_scaffold, get_python2_retire_msg, prettify_json_file, validate_json_file
...
def main_hrun():
    ...
    parser.add_argument("--no-html-report", action="store_true", default=False, help="do not generate html report.")
    parser.add_argument(
        "--html-report-name", help="specify html report name, only effective when generating html report."
    )
    ...
    parser.add_argument("--dot-env-path", help="Specify .env file path, which is useful for keeping sensitive data.")
    parser.add_argument(
    ...
    if not args.no_html_report:
        runner.gen_html_report(html_report_name=args.html_report_name, html_report_template=args.html_report_template)
