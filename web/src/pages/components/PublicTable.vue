...
原文链接：https://blog.csdn.net/qq_37346639/article/details/115556605
 ...
 <template>
	<div>
		<el-table size="medium" :data="tableData" :stripe="true" :border="false" :fit="true" :show-header="true"
				...
				<template v-slot="{row,$index}">
					...
					<span v-if="column.isOperation" v-for="(operations, index) in column.operation" :key="index">
						<el-button v-if="operations.isShow(row,$index)" :icon="operations.icon" :type="operations.type"
						 @click="operations.buttonClick(row,$index)" :style="{color:operations.color, marginLeft:0}" :circle="operations.circle" size="mini">
							{{operations.label}}
						</el-button>
					</span>
				</template>
		...
		<div class="page_div" :style="{textAlign: pageObj.position || 'center'}">
			<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
				...
				layout="total, prev, pager, next, jumper, sizes" :total="pageObj.total">
			</el-pagination>
		</div>
	</div>
</template>
